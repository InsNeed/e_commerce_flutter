...
class CartProvider extends ChangeNotifier {
  ...
  void updateCart(CartModel item, int newQuantity) {
    var quantity = item.quantity + newQuantity;
    flutterCart.updateQuantity(item.productId, item.variants, quantity);
    notifyListeners();
  }
      ...
      if (response.isOk) {
        ...
        if (apiResponse.success == true) {
          ...
          SnackBarHelper.showErrorSnackBar(
              'Filed to validate Coupon :${apiResponse.message}');
        }
      } else {
        SnackBarHelper.showSuccessSnackBar(
  ...
  double getCouponDiscountAmount(Coupon coupon) {
    ...
    if (discountType == 'fixed') {
      discountAmout = coupon.discountAmount ?? 0;
    ...
    try {
      ...
      if (response.isOk) {
        ...
        if (apiResponse.success == true) {
          ...
          SnackBarHelper.showErrorSnackBar(
              'Filed to add Order: ${apiResponse.message}');
        }
      } else {
        SnackBarHelper.showErrorSnackBar(
            'Error ${response.body?['message'] ?? response.statusText}');
